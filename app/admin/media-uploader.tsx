...
export default function MediaUploader({ articles, games }: { articles: Target[]; games: Target[] }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const form = event.currentTarget; const data = new FormData(form); const file = data.get("file"); const target = String(data.get("target") || "");
    if (!(file instanceof File) || !target) { setMessage("Choose an image and one target record."); return; }
    setBusy(true); setMessage("Checking image dimensionsâ€¦");
    try {
      ...
      data.set("width", String(image.naturalWidth)); data.set("height", String(image.naturalHeight)); URL.revokeObjectURL(url);
      data.delete("target"); if (target.startsWith("article:")) data.set("articleId", target.slice(8)); else data.set("gameId", target.slice(5));
      const response = await fetch("/api/admin/media/upload", { method: "POST", body: data }); const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Upload could not be saved.");
      form.reset(); setMessage("Private media record created. Review and approve it before it can appear publicly.");
  ...
  return <form className="director-console-form" onSubmit={submit}>
    ...
    <select name="target" required defaultValue=""><option value="" disabled>Attach toâ€¦</option>{articles.map((article) => <option key={`article:${article.id}`} value={`article:${article.id}`}>Article: {article.title}</option>)}{games.map((game) => <option key={`game:${game.id}`} value={`game:${game.id}`}>Game: {game.title}</option>)}</select>
    ...
    <button className="director-console-primary" disabled={busy}>{busy ? "Uploadingâ€¦" : "Upload as pending media"}</button>
