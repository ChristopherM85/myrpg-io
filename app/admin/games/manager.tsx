...
export default function GameManager({ role, games, sources, calendarItems }: { role: string; games: RecordItem[]; sources: RecordItem[]; calendarItems: RecordItem[] }) {
  ...
  const [editingCalendar, setEditingCalendar] = useState<RecordItem | null>(null);
  ...
  const reviewPriority: Record<string, number> = { Approve: 0, Edit: 1, Hold: 2 };
  const reviewNext = candidates.map((game) => {
    const blockers = gameBlockers(game, approvedDomains); const hard = blockers.filter((item) => !item.includes("Missing Owner review decision") && !item.includes("unconfirmed"));
    const recommendation = hard.length ? "Edit" : game.sourceConfidence === "pending" || game.status === "announced" ? "Hold" : "Approve";
    return { game, blockers, recommendation };
  }).sort((a, b) => reviewPriority[a.recommendation] - reviewPriority[b.recommendation] || a.game.name.localeCompare(b.game.name));
  return <main className="console">
    ...
    {editing && <section><h2>Correct verified facts: {editing.name}</h2><p className="console-sub">Save only facts supported by the approved official source. This creates an audit event and never publishes.</p><form className="source-form" onSubmit={(event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); send({ kind: "game", action: "correct", id: editing.id, ...values }); }}>
      {["status", "platforms", "businessModel", "combat", "setting", "focus", "activity", "timeCommitment", "releaseDate", "officialUrl", "sourceUrl", "factCheckedAt", "sourceConfidence"].map((name) => <input key={name} name={name} defaultValue={editing[name] || ""} placeholder={name} />)}
      <select name="releaseDateConfidence" defaultValue={editing.releaseDateConfidence || "unconfirmed"} aria-label="Corrected release-date confidence"><option value="confirmed">confirmed</option><option value="estimated">estimated</option><option value="unconfirmed">unconfirmed</option></select><textarea name="directorySummary" defaultValue={editing.directorySummary || ""} placeholder="Factual directory summary" required /><button>Save verified correction</button><button type="button" onClick={() => setEditing(null)}>Cancel</button>
    </form></section>}
    <section><h2>Review next</h2><p className="console-sub">Prioritized from current official-source coverage. “Hold” means the official record still has unresolved information; it is not a rejection.</p><div className="console-grid">{reviewNext.map(({ game, blockers, recommendation }) => <article className="console-card" key={`next-${game.id}`}><small>{recommendation.toUpperCase()} · {game.sourceConfidence || "pending"} confidence</small><h3>{game.name}</h3><p>{recommendation === "Approve" ? "Complete enough for an Owner decision." : recommendation === "Edit" ? blockers.join(" · ") : "Hold until the official details are more complete."}</p><p><a href={game.sourceUrl} target="_blank" rel="noopener noreferrer">Official source</a></p><div className="row"><button type="button" onClick={() => window.location.assign(`/admin/preview/game/${encodeURIComponent(game.id)}`)}>Review packet</button><button type="button" onClick={() => setEditing(game)}>Correct facts</button></div></article>)}</div></section>
    ...
    {editingCalendar && <section><h2>Correct verified calendar facts: {editingCalendar.title}</h2><form className="source-form" onSubmit={(event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); send({ kind: "calendar", action: "correct", id: editingCalendar.id, ...values }); }}><input name="title" defaultValue={editingCalendar.title} required /><input name="dateLabel" defaultValue={editingCalendar.dateLabel} required /><input name="sourceUrl" defaultValue={editingCalendar.sourceUrl} required /><input name="factCheckedAt" defaultValue={editingCalendar.factCheckedAt} required /><select name="dateConfidence" defaultValue={editingCalendar.dateConfidence}><option value="confirmed">confirmed</option><option value="estimated">estimated</option><option value="unconfirmed">unconfirmed</option></select><button>Save verified correction</button><button type="button" onClick={() => setEditingCalendar(null)}>Cancel</button></form></section>}
    <section><h2>Release calendar review</h2><p className="console-sub">Calendar drafts require an approved source, current fact check, a resolved date confidence, and Owner approval.</p><div className="console-grid">{calendarItems.filter((item) => !item.published && item.reviewStatus !== "archived").map((item) => { const blockers = calendarBlockers(item, approvedDomains); return <article className="console-card" key={item.id}><small>{String(item.reviewStatus).toUpperCase()} · {item.dateConfidence}</small><h3>{item.title}</h3><p>{item.dateLabel}</p><small>Checked {item.factCheckedAt?.slice(0, 10)} · <a href={item.sourceUrl} target="_blank" rel="noopener noreferrer">Official source</a></small><p className={blockers.length ? "console-message" : "console-sub"}>{blockers.length ? `Readiness: ${blockers.join(" · ")}` : "Ready for Owner publication."}</p><div className="row"><button type="button" onClick={() => window.location.assign(`/admin/preview/calendar/${encodeURIComponent(item.id)}`)}>Preview packet</button><button type="button" onClick={() => setEditingCalendar(item)}>Correct facts</button><button onClick={() => send({ kind: "calendar", action: "approve", id: item.id })} disabled={role !== "owner"}>Approve</button><button onClick={() => send({ kind: "calendar", action: "reject", id: item.id })}>Reject</button><button onClick={() => send({ kind: "calendar", action: "archive", id: item.id })}>Archive</button><button onClick={() => send({ kind: "calendar", action: "publish", id: item.id })} disabled={role !== "owner" || blockers.length > 0}>Publish</button></div></article>; })}</div></section>
