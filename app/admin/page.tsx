...
export default async function AdminPage() {
  ...
  let role = "owner";
  let data: any = { articles: [], games: [], sources: [], runs: [], audits: [], media: [], searchStatuses: [], editorialPlans: [], cadence: { planned: 0, ready: 0, published: 0, blocked: 0, stale: 0 }, visualCoverage: { approved: 0, fallback: 0, pending: 0, metadata: 0 }, launchBatch: { ready: 0, correction: 0, hold: 0 }, overview: { articles: 0, games: 0, calendar: 0, published: 0 }, health: { records: [], seoIssues: 0, approvedSources: 0, nextAction: "Add an approved source before preparing a private record.", seoItems: [] }, launchGate: { published: { articles: 0, games: 0, calendar: 0 }, freshness: { current: 0, stale: 0, thresholdDays: 30 }, sitemap: { eligible: 0, blocked: 0 }, blockers: ["D1 data is unavailable; restore storage before assessing launch readiness."], routes: [], checkedAt: new Date().toISOString(), verificationNote: "Storage was unavailable for this structural check. Restore D1 before assessing launch readiness." }, settings: { simulation: true, promotion: true, daily: 800, perJob: 125, stop: false } };
  try {
    ...
    const [articleRows, allArticles, sourceRows, runRows, auditRows, mediaRows, budgets, settings, gameRows, calendarRows, searchRows, planRows] = await Promise.all([db.select().from(articles).orderBy(desc(articles.createdAt)).limit(16), db.select().from(articles), db.select().from(sources), db.select().from(agentRuns).orderBy(desc(agentRuns.createdAt)).limit(20), db.select().from(auditEvents).orderBy(desc(auditEvents.createdAt)).limit(20), db.select().from(mediaAssets).orderBy(desc(mediaAssets.createdAt)), db.select().from(budgetPolicies).limit(1), db.select().from(siteSettings).limit(1), db.select().from(games), db.select().from(calendarItems), db.select().from(searchEngineStatuses), db.select().from(editorialPlans).orderBy(desc(editorialPlans.proposedDate))]);
