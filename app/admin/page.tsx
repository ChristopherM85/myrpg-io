...
import { agentRuns, articles, auditEvents, budgetPolicies, calendarItems, editorialPlans, gameSubmissions, games, mediaAssets, searchEngineStatuses, siteSettings, sources, sourceWatchlist, users } from "../../db/schema";
...
export default async function AdminPage() {
  ...
  try {
    ...
    const feedIntake = runRows.filter((run) => run.status === "evidence_ready" && run.itemId?.startsWith("feed:")).map((run) => { let output: any = {}; try { output = JSON.parse(run.outputJson || "{}"); } catch { /* Invalid items remain omitted from action recommendations. */ } return { id: run.id, title: output.title || "Untitled official-feed item", sourceUrl: output.sourceUrl || "", sourceDate: output.sourceDate || null, sourceDomain: output.sourceDomain || "Approved source", confidence: output.confidence || "low", nextStep: output.nextStep || "Owner evidence review required." }; });
