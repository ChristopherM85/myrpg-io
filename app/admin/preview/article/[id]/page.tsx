...
export default async function ArticlePreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const minimumWords = article.retrospective ? 140 : 120; const maximumWords = article.retrospective ? 220 : 180;
  ...
  const blockers = [!source?.approved && "Official source is not approved", !cache && "Duplicate-check record is missing", !article.factCheckedAt && "Fact-check date is missing", !fresh && "Fact-check date is stale", wordCount < minimumWords || wordCount > maximumWords ? `Draft must be ${minimumWords}–${maximumWords} words` : false, article.retrospective && !article.sourceDate && "Original official-source date is missing", article.retrospective && !article.gamerTakeaway && "Why this still matters takeaway is missing", article.status !== "review" && "Draft is not in human review", !ownerApproved && "Explicit Owner review decision is required"].filter((item): item is string => Boolean(item));
  return <main style={shell}>
    <p style={kicker}>OWNER-ONLY PREVIEW · NOINDEX{article.retrospective ? " · RETROSPECTIVE" : ""}</p><h1 style={heading}>{article.title}</h1>
