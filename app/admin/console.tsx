...
export default function Console({ role, articles, games, sources, runs, audits, media, settings, overview, visualCoverage, launchBatch, health, launchGate }: {
  role: string; articles: Item[]; games: Item[]; sources: Item[]; runs: Item[]; audits: Item[]; media: Item[]; settings: Settings; overview: { articles: number; games: number; calendar: number; published: number }; visualCoverage: { approved: number; fallback: number; pending: number; metadata: number; items?: { id: string; kind: string; title: string; state: string; assetId: string | null; previewHref: string }[] }; launchBatch: { ready: number; correction: number; hold: number }; health: { records: { kind: string; factCheckedAt?: string | null; approvedMedia: boolean; fallback: boolean; valid: boolean }[]; seoIssues: number; approvedSources: number; nextAction: string; seoItems: { name: string; kind: string; issues: string[] }[] }; launchGate: LaunchGate;
}) {
  ...
  return <main className="director-console">
    ...
    <section className={styles.section}>
      ...
      <div className={styles.grid}>{visualCoverage.items?.length ? visualCoverage.items.map((item) => <article className={styles.card} key={`${item.kind}:${item.id}`}><small>{item.kind.toUpperCase()} · {item.state.toUpperCase()}</small><h3>{item.title}</h3><p>{item.state === "MyRPG fallback in use" ? "A labelled MyRPG editorial graphic is public until licensed media is approved." : "Review the media record before changing public coverage."}</p><div className={styles.row}><a href={item.previewHref}>Open preview packet</a>{item.assetId && <a href={`/admin/media/${item.assetId}`}>Open media record</a>}</div></article>) : <div className={styles.empty}>No published articles or games require visual coverage review yet.</div>}</div>
    </section>
