...
type GameSubmission = { id: string; gameName: string; studioName?: string | null; officialUrl: string; sourceUrl?: string | null; description: string; coverageLane: string; lifecycleStatus?: string | null; platforms?: string | null; submitterNote?: string | null; imageR2Key?: string | null; imageContentType?: string | null; imageWidth?: number | null; imageHeight?: number | null; imageAltText?: string | null; rightsConfirmed: boolean; promotionalAssetPermission: boolean; officialCrawlPermission: boolean; promotionalCapturePermission: boolean; editorialUsePermission: boolean; permissionsContactEmail?: string | null; status: string; createdAt: string };
...
export default function Console({ role, articles, games, sources, sourceWatchlist, runs, audits, media, submissions, searchStatuses, editorialPlans, feedIntake, dailyEditorial, cadence, settings, overview, visualCoverage, launchBatch, health, launchGate }: {
  ...
  return <main className="director-console">
    ...
    {role === "owner" && <section className="director-console-section">
      <h2>Tiered feed intake</h2><p className="director-console-helper">Primary official feeds may capture private evidence. Discovery feeds create private verification leads only: they cannot become evidence, drafts, citations, or public content. The scheduler reads at most four primary feeds (two items each) and four discovery feeds (one lead each) per day.</p>
      <div className="director-console-row"><button className="director-console-primary" disabled={busy} onClick={() => send({}, "/api/admin/feed-run")}>Run configured feeds now</button><span className="director-console-helper">Owner-triggered, zero-cost, private-only check. It uses the same daily feed limits and cannot publish.</span></div>
      <form className="director-console-form" onSubmit={(event) => { event.preventDefault(); const values = new FormData(event.currentTarget); send({ kind: "source_feed", action: "configure", id: values.get("sourceId"), feedUrl: values.get("feedUrl"), feedTier }); }}><select aria-label="Feed authority tier" value={feedTier} onChange={(event) => setFeedTier(event.target.value === "discovery" ? "discovery" : "primary")}><option value="primary">Primary official evidence</option><option value="discovery">Editorial discovery leads</option></select><select name="sourceId" required><option value="">{feedTier === "primary" ? "Approved primary official source" : "Approved discovery source"}</option>{sources.filter((source) => source.approved && (source.sourceRole || "primary") === feedTier).map((source) => <option key={source.id} value={source.id}>{source.label || source.domain} ({source.domain})</option>)}</select><input name="feedUrl" type="url" placeholder={feedTier === "primary" ? "https://official-domain.example/feed/" : "https://editorial-domain.example/rss/"} required /><button className="director-console-primary" disabled={busy}>Configure {feedTier === "primary" ? "official" : "discovery"} feed</button></form>
      <div className="director-console-grid" style={{ marginTop: 14 }}>{sourceWatchlist.filter((watch) => watch.checkMode === "approved_feed").map((watch) => { const source = sources.find((item) => item.id === watch.sourceId); let feedUrl = ""; try { feedUrl = JSON.parse(watch.note || "{}").feedUrl || ""; } catch {} const tier = watch.feedTier === "discovery" ? "DISCOVERY / LEADS ONLY" : "PRIMARY / PRIVATE EVIDENCE"; return <article className="director-console-card" key={watch.sourceId}><small>{watch.status === "ready" ? `${tier} / OPERATIONAL` : `${tier} / HELD`}</small><h3>{source?.label || source?.domain || "Configured source"}</h3><p>{feedUrl || "Feed URL unavailable"}</p><button disabled={busy || watch.status === "hold"} onClick={() => action("source_feed", "pause", watch.sourceId)}>Pause feed</button></article>; })}</div>
      <div className="director-console-grid" style={{ marginTop: 14 }}>{feedIntake.slice(0, 12).map((item) => <article className="director-console-card" key={item.id}><small>{item.tier === "primary" ? `PRIVATE OFFICIAL EVIDENCE / ${item.confidence.toUpperCase()}` : "PRIVATE DISCOVERY LEAD / VERIFY OFFICIAL SOURCE"}</small><h3>{item.title}</h3><p>{item.sourceDomain}{item.sourceDate ? ` · ${item.sourceDate}` : " · source date unavailable"}</p><p><a href={item.sourceUrl} target="_blank" rel="noreferrer">{item.tier === "primary" ? "Official item" : "Discovery item"}</a></p><p>{item.nextStep}</p></article>)}</div>
      {!feedIntake.length && <div className="director-console-empty" style={{ marginTop: 14 }}>No captured feed items yet. Primary feeds create private evidence; discovery feeds create a private lead requiring a direct official source before any editorial work.</div>}
    </section>}
    ...
    <section className="director-console-section">
      <h2>Creator game submissions</h2><p className="director-console-helper">Public submissions arrive as private leads only. Jordan Hale&rsquo;s Live Service &amp; Games Desk can use them as a starting point, but only approved official evidence and an Owner decision can create or publish coverage.</p>
      <div className="director-console-grid">{submissions.length ? submissions.map((submission) => <article className={`director-console-card ${submission.status === "new" ? "director-console-card-approval" : submission.status === "needs_info" ? "director-console-card-information" : "director-console-card-operational"}`} key={submission.id}><p className={`director-console-status ${submission.status === "new" ? "director-console-status-approval" : submission.status === "needs_info" ? "director-console-status-information" : "director-console-status-operational"}`}>{submission.status === "new" ? "OWNER REVIEW REQUIRED" : submission.status.replaceAll("_", " ").toUpperCase()}</p><h3>{submission.gameName}</h3><p>{submission.studioName || "Studio not specified"} · {submission.coverageLane.replaceAll("_", " ")}{submission.lifecycleStatus ? ` · ${submission.lifecycleStatus.replaceAll("_", " ")}` : ""}</p><p>{submission.description}</p><p><a href={submission.officialUrl} target="_blank" rel="noreferrer">Official website</a>{submission.sourceUrl && <> · <a href={submission.sourceUrl} target="_blank" rel="noreferrer">Official source</a></>}</p><p>{submission.platforms || "Platforms not supplied."}</p>{submission.imageR2Key ? <p>Private creator image: {submission.imageWidth}×{submission.imageHeight} · rights attestation {submission.rightsConfirmed ? "received" : "missing"}. It is not publicly served or attached to media.</p> : <p>No creator image attached; use a labelled MyRPG fallback if a future record is published.</p>}<div className="director-console-row">{role === "owner" && <><button disabled={busy} onClick={() => action("game_submission", "reviewed", submission.id)}>Mark ready for source review</button><button disabled={busy} onClick={() => action("game_submission", "needs_info", submission.id)}>Needs more information</button><button disabled={busy} onClick={() => action("game_submission", "archive", submission.id)}>Archive</button></>}</div></article>) : <div className="director-console-empty">No public game submissions yet. Share <a href="/submit-game">/submit-game</a> with creators; submissions stay private and no account is required.</div>}</div>
    </section>
    ...
    <section className="director-console-section">
      ...
      <div className="director-console-grid">{sources.map((source) => <article className="director-console-card" key={source.id}><small>{(source.sourceRole || "primary") === "discovery" ? "DISCOVERY ONLY / CANNOT CITE OR DRAFT" : "PRIMARY OFFICIAL SOURCE"}</small><h3>{source.label}</h3><p>{source.domain}</p><button disabled={busy} onClick={() => action("source", source.approved ? "disable" : "approve", source.id)}>{source.approved ? "Disable" : "Approve"}</button></article>)}</div>
    </section>
