...
export default function Console({ role, articles, games, sources, sourceWatchlist, runs, audits, media, submissions, searchStatuses, editorialPlans, feedIntake, dailyEditorial, cadence, settings, overview, visualCoverage, launchBatch, health, launchGate }: {
  ...
  return <main className="director-console">
    ...
    {role === "owner" && <section className="director-console-section">
      <h2>Approved-source feed intake</h2><p className="director-console-helper">RSS and Atom only. The scheduler reads at most four Owner-configured, approved-domain feeds and two items per feed per day. It does not follow story links, crawl sites, draft, or publish. Each new item stays private until an Owner completes evidence review.</p>
      <form className="director-console-form" onSubmit={(event) => { event.preventDefault(); const values = new FormData(event.currentTarget); send({ kind: "source_feed", action: "configure", id: values.get("sourceId"), feedUrl: values.get("feedUrl") }); }}><select name="sourceId" required><option value="">Approved official source</option>{sources.filter((source) => source.approved).map((source) => <option key={source.id} value={source.id}>{source.label || source.domain} ({source.domain})</option>)}</select><input name="feedUrl" type="url" placeholder="https://approved-domain.example/rss.xml" required /><button className="director-console-primary" disabled={busy}>Configure bounded feed</button></form>
      <div className="director-console-grid" style={{ marginTop: 14 }}>{sourceWatchlist.filter((watch) => watch.checkMode === "approved_feed").map((watch) => { const source = sources.find((item) => item.id === watch.sourceId); let feedUrl = ""; try { feedUrl = JSON.parse(watch.note || "{}").feedUrl || ""; } catch {} return <article className="director-console-card" key={watch.sourceId}><small>{watch.status === "ready" ? "APPROVED FEED / OPERATIONAL" : "FEED HELD"}</small><h3>{source?.label || source?.domain || "Approved source"}</h3><p>{feedUrl || "Feed URL unavailable"}</p><button disabled={busy || watch.status === "hold"} onClick={() => action("source_feed", "pause", watch.sourceId)}>Pause feed</button></article>; })}</div>
      <div className="director-console-grid" style={{ marginTop: 14 }}>{feedIntake.slice(0, 8).map((item) => <article className="director-console-card" key={item.id}><small>PRIVATE FEED EVIDENCE / {item.confidence.toUpperCase()}</small><h3>{item.title}</h3><p>{item.sourceDomain}{item.sourceDate ? ` · ${item.sourceDate}` : " · source date unavailable"}</p><p><a href={item.sourceUrl} target="_blank" rel="noreferrer">Official item</a></p><p>{item.nextStep}</p></article>)}</div>
      {!feedIntake.length && <div className="director-console-empty" style={{ marginTop: 14 }}>No captured feed items yet. Configure one approved official RSS or Atom URL; the next daily run will capture only its newest bounded items privately.</div>}
    </section>}
