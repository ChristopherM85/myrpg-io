...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><PublicHeader /><main className="article-page">
    ...
    <p className="article-kicker">{article.retrospective ? "RETROSPECTIVE · OFFICIAL UPDATE ARCHIVE · " : ""}AI-ASSISTED · HUMAN-REVIEWED</p>
    <h1>{article.title}</h1>
    ...
    <section className="article-notes"><div className="article-notes-heading"><p>VERIFIED COVERAGE</p><h2>Source &amp; editorial notes</h2></div><a href={`/writers#${MAYA.slug}`} className="article-author"><div className="article-author-portrait"><WriterPortrait writer={MAYA} compact /></div><span><strong>{MAYA.name}</strong><b>{MAYA.title}</b><small>MyRPG editorial persona · AI-assisted, human-reviewed</small></span><span className="article-author-arrow" aria-hidden="true">→</span></a><dl className="article-fact-row">{article.retrospective && <div><dt>Originally announced</dt><dd><time dateTime={article.sourceDate || undefined}>{displayDate(article.sourceDate)}</time></dd></div>}<div><dt>Published by MyRPG</dt><dd><time dateTime={article.publishedAt || undefined}>{displayDate(article.publishedAt)}</time></dd></div><div><dt>Last fact-check</dt><dd><time dateTime={article.factCheckedAt || undefined}>{displayDate(article.factCheckedAt)}</time></dd></div><div><dt>Primary source</dt><dd><a href={article.sourceUrl} rel="noopener noreferrer" target="_blank">Official source <span aria-hidden="true">↗</span></a></dd></div></dl>{article.retrospective && article.gamerTakeaway && <p className="article-transparency"><strong>Why this still matters:</strong> {article.gamerTakeaway}</p>}<p className="article-transparency">This is an AI-assisted, human-reviewed factual summary. MyRPG does not publish autonomous coverage.</p></section>
    ...
    <ExploreNext links={[...(relatedGame ? [{ href: `/games/${relatedGame.slug}`, label: `Explore ${relatedGame.name}`, note: "Open the factual game profile." }] : []), { href: "/news", label: "More MMO news", note: "Browse published, source-linked updates." }, { href: "/find-my-mmo", label: "Find My MMO", note: "Use verified game fields to narrow your options." }]} />
