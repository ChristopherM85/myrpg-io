...
export function EditorialVisual({ title, category = "MMO intelligence", label = "Editorial brief", eager = true, image, themeKey }: Props) {
  ...
  return <figure className="editorial-visual editorial-visual-fallback" aria-label="MyRPG editorial graphic — not gameplay"><div className="editorial-visual-frame"><img src={theme.src} alt={theme.alt} width={theme.width} height={theme.height} loading={eager ? "eager" : "lazy"} style={{ objectPosition: "position" in theme ? theme.position : "center" }} /><div aria-hidden="true" /></div><figcaption><span>MyRPG editorial graphic — not gameplay.</span><span>{theme.label}</span></figcaption><div className="editorial-visual-fallback-copy"><small>MYRPG EDITORIAL GRAPHIC — NOT GAMEPLAY</small><h2>{title}</h2><p>{category} · {label}</p></div></figure>;
}
