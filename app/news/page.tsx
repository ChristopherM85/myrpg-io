...
export default async function News() {
  ...
  return <PublicPage><nav aria-label="Breadcrumb"><a href="/">Home</a> / News</nav><p style={eyebrow}>MYRPG / NEWS</p><h1 style={heading}>MMO news</h1><p style={muted}>Source-linked and human-reviewed. No autonomous publishing.</p>{rows.length ? <section style={{ marginTop: 28 }}>{rows.map((article) => { const gameSlug = relations.get(article.id); const visual = visuals.get(article.id); return <article key={article.id} style={row}><p style={label}>MAYA CHEN · SIGNAL EDITOR</p><h2><a href={`/articles/${article.slug}`} style={link}>{article.title}</a></h2>{visual ? <img src={visual.assetUrl || visual.sourceUrl || ""} alt={visual.altText} width={visual.width || 1200} height={visual.height || 675} style={image} /> : <div style={fallback}>MyRPG editorial graphic — not gameplay</div>}<p style={muted}>{article.summary}</p><small>Published: {article.publishedAt?.slice(0, 10)} · Recently fact-checked: {article.factCheckedAt?.slice(0, 10)} · <a href={article.sourceUrl} target="_blank" rel="noopener noreferrer" style={link}>Official source</a>{gameSlug && <> · <a href={`/games/${gameSlug}`} style={link}>Related game: {gameNames.get(gameSlug)}</a></>} · AI-assisted, human-reviewed</small></article>; })}</section> : <section style={empty}><h2>News is reviewed before it goes live</h2><p>No published editorial coverage is available yet. MyRPG publishes only source-linked, human-reviewed MMO coverage — never simulations, scraped recaps, or unreviewed stories.</p></section>}</PublicPage>;
}
