...
export default async function MmoRadar() {
  ...
  try { const db = getDb(); const [gameRows, assets] = await Promise.all([db.select().from(games).where(eq(games.published, true)).orderBy(desc(games.factCheckedAt)), db.select().from(mediaAssets).where(eq(mediaAssets.status, "approved"))]); published = gameRows; for (const asset of assets.filter((item) => item.gameId && ["directory-card", "lead"].includes(item.placement)).sort((a) => a.placement === "directory-card" ? -1 : 1)) if (asset.gameId && !visuals.has(asset.gameId)) visuals.set(asset.gameId, asset); } catch { /* The public empty state deliberately reveals no internal failure. */ }
  ...
  return <PublicPage><nav aria-label="Breadcrumb"><a href="/">Home</a> / MMO Radar</nav><p style={eyebrow}>MYRPG / MMO INTELLIGENCE</p><h1 style={heading}>MMO Radar</h1><p style={lede}>A factual overview of owner-published MMO profiles. Every link below uses visible structured data, official source citations, and human approval—never rankings or popularity claims.</p>
    ...
    <RadarSection title="Recently fact-checked" note="Ordered by the stored fact-check date; this is not a claim about real-time game activity." rows={published.slice(0, 6)} empty="No published game profiles have a stored fact-check date yet." visuals={visuals} />
    <RadarSection title="Confirmed upcoming releases" note="Only profiles with an owner-entered confirmed date appear here." rows={confirmed.slice(0, 5)} empty="No owner-published profiles currently have a confirmed upcoming release date." release visuals={visuals} />
    ...
    <RadarSection title="Browser MMORPGs" note="Classified only when the published platform field says browser." rows={published.filter((game) => includes(game.platforms, "browser")).slice(0, 5)} empty="No published browser MMORPG profiles yet." visuals={visuals} />
    ...
    <ExploreNext links={[{ href: "/official-updates", label: "Official updates", note: "Read published source-linked announcements." }, { href: "/games", label: "Browse games", note: "Open the full factual directory." }, { href: "/calendar", label: "Release calendar", note: "See owner-published date records." }, { href: "/compare", label: "Compare profiles", note: "Place up to three visible fields side by side." }, { href: "/find-my-mmo", label: "Find My MMO", note: "Match verified fields to your preferences." }]} />
  </PublicPage>;
}

function RadarSection({ title, note, rows, empty, release = false, visuals }: { title: string; note: string; rows: any[]; empty: string; release?: boolean; visuals: Map<string, any> }) { return <section style={section}><p style={eyebrow}>FACTUAL OVERVIEW</p><h2 style={h2}>{title}</h2><p style={muted}>{note}</p>{rows.length ? <div style={grid}>{rows.map((game) => <article key={game.id} style={card}><EditorialVisual title={game.name} category={game.setting || "MMO"} label="Radar profile" image={visuals.get(game.id)} themeKey={game.editorialGraphic} eager={false} /><small>{String(game.status).toUpperCase()}</small><h3><a href={`/games/${game.slug}`}>{game.name}</a></h3><p style={muted}>{game.platforms} · {game.businessModel}</p><p style={muted}>{release ? `Confirmed release: ${game.releaseDate}` : `Fact-checked: ${day(game.factCheckedAt)}`}</p><a href={`/games/${game.slug}`} style={accent}>Open profile →</a></article>)}</div> : <article style={emptyCard}><h3>Nothing to list yet</h3><p>{empty}</p><a href="/games" style={accent}>Browse the directory →</a></article>}</section>; }

export async function generateMetadata({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) { const query = await searchParams; const title = "MMO Radar | MyRPG.IO"; const description = "A human-approved factual overview of published MMO profiles, release dates, platforms, and play styles."; return { title, description, alternates: { canonical: `${base}/mmo-radar` }, openGraph: { title, description, url: `${base}/mmo-radar` }, robots: Object.keys(query).length ? { index: false, follow: true } : { index: true, follow: true } }; }
