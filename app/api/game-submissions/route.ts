...
export async function POST(request: Request) {
  ...
  await db.insert(gameSubmissions).values({ id, gameName, studioName: studioName || null, officialUrl, sourceUrl: sourceUrl || null, description, coverageLane, lifecycleStatus: lifecycleStatus || null, platforms: platforms || null, submitterNote: submitterNote || null, imageR2Key, imageContentType: imageType, imageWidth, imageHeight, imageAltText: imageAltText || null, rightsConfirmed, promotionalAssetPermission, submitterHash, status: "new", createdAt, updatedAt: createdAt });
  await db.insert(auditEvents).values({ id: crypto.randomUUID(), actorEmail: "public-submission", action: "game_submission_received", entityType: "game_submission", entityId: id, details: JSON.stringify({ coverageLane, hasImage: Boolean(imageR2Key), promotionalAssetPermission, sourceProvided: Boolean(sourceUrl), privateOnly: true, autoCapture: false, autoPublish: false }), createdAt });
