...
export async function POST(request: Request) {
  ...
  const time = stamp();
  if(kind==="game"){
    if(body.action==="create"){const fields=[body.name,body.slug,body.status,body.platforms,body.businessModel,body.combat,body.setting,body.focus,body.releaseDate,body.officialUrl,body.sourceUrl,body.factCheckedAt,body.directorySummary];if(fields.some(x=>!String(x||"").trim())||!String(body.slug).match(/^[a-z0-9-]+$/))return Response.json({error:"All factual fields and the directory summary are required; slug must be lowercase letters, numbers, and hyphens."},{status:400});let host="";try{host=new URL(String(body.sourceUrl)).hostname.replace(/^www\./,"")}catch{return Response.json({error:"Use a complete official source URL."},{status:400})}const source=(await db.select().from(sources).where(eq(sources.domain,host)).limit(1))[0];if(!source?.approved)return Response.json({error:"Source domain must be approved first."},{status:400});const releaseDateConfidence=["confirmed","estimated","unconfirmed"].includes(body.releaseDateConfidence||"")?body.releaseDateConfidence!:"unconfirmed";await db.insert(games).values({id:id(),slug:body.slug!,name:body.name!,status:body.status!,platforms:body.platforms!,businessModel:body.businessModel!,combat:body.combat!,setting:body.setting!,focus:body.focus!,activity:body.activity?.trim()||null,timeCommitment:body.timeCommitment?.trim()||null,releaseDate:body.releaseDate!,releaseDateConfidence,officialUrl:body.officialUrl!,sourceUrl:body.sourceUrl!,factCheckedAt:body.factCheckedAt!,directorySummary:body.directorySummary!,sourceConfidence:"high",reviewStatus:"draft",published:false,createdAt:time,updatedAt:time});await audit(db,email,"game_created","game",body.slug);return Response.json({ok:true})}
  ...
  if (kind === "article" && body.action === "intake") {
    ...
    const intake = { manualIntake: true, noModel: true, normalizedUrl, sourceDate: sourceDate.toISOString(), gameSlug: relatedGame.slug, gameName: relatedGame.name, citation: normalizedUrl, sourceConfidence: "high", duplicate: "clear", validation: "pass", gamerTakeaway: "Check the official announcement for requirements, timing, and scope before making plans.", recommendation: "approve after Owner confirms the source, factual summary, citation, date, and fallback media." };
