...
const isPrimarySource = (source?: { approved?: boolean; sourceRole?: string | null }) => Boolean(source?.approved && (source.sourceRole || "primary") === "primary");
...
export async function POST(request: Request) {
  ...
  if (kind === "source_feed") {
    ...
    const feedTier = body.feedTier === "discovery" ? "discovery" : "primary";
    if (!source?.approved || (source.sourceRole || "primary") !== feedTier) return Response.json({ error: feedTier === "primary" ? "An approved primary official source is required for evidence intake." : "An approved discovery source is required for verification leads." }, { status: 409 });
    ...
    const itemLimit = feedTier === "primary" ? 2 : 1;
    const note = JSON.stringify({ feedUrl, itemLimit, mode: feedTier === "primary" ? "bounded_daily_rss_atom" : "bounded_daily_discovery_rss_atom", privateOnly: true, autoPublish: false, noDraft: feedTier === "discovery" });
    if (existing) await db.update(sourceWatchlist).set({ checkMode: "approved_feed", feedTier, status: "ready", note, updatedAt: time }).where(eq(sourceWatchlist.id, existing.id));
    else await db.insert(sourceWatchlist).values({ id: id(), sourceId: source.id, checkMode: "approved_feed", feedTier, status: "ready", note, createdAt: time, updatedAt: time });
    await audit(db, email, feedTier === "primary" ? "approved_feed_configured" : "discovery_feed_configured", "source_feed", source.id, { feedUrl, feedTier, maxItemsPerDay: itemLimit, privateOnly: true, autoPublish: false, noDraft: feedTier === "discovery" });
    return Response.json({ ok: true });
  }
  ...
  if (kind === "source_watch") {
    ...
    const source = (await db.select().from(sources).where(eq(sources.id, body.id)).limit(1))[0];
    if (!isPrimarySource(source)) return Response.json({ error: "Only a primary approved official source can enter the evidence watchlist." }, { status: 409 });
    const existing = (await db.select().from(sourceWatchlist).where(eq(sourceWatchlist.sourceId, source.id)).limit(1))[0];
  ...
  if (kind === "timeline") {
    ...
    if (body.action === "create") {
      ...
      const host = new URL(sourceUrl).hostname; if (!isPrimarySource((await db.select().from(sources).where(eq(sources.domain, host)).limit(1))[0])) return Response.json({ error: "A primary approved official source is required." }, { status: 400 });
  ...
  if(kind==="game"){
    if(body.action==="create"){
      ...
      if(!isPrimarySource(source)) return Response.json({error:"A primary approved official source is required."},{status:400});
  ...
  if(kind==="calendar"){
    if(body.action==="create"){
      ...
      const host=new URL(normalizedSourceUrl).hostname; if(!isPrimarySource((await db.select().from(sources).where(eq(sources.domain,host)).limit(1))[0])) return Response.json({error:"Calendar source must be a primary approved official source."},{status:400});
  ...
  if (kind === "article" && body.action === "intake") {
    ...
    if (!isPrimarySource(source)) return Response.json({ error: "That source is not a primary approved official source in the Source Registry." }, { status: 400 });
